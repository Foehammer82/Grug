...
Registers ``get_character_sheet`` and ``search_character_knowledge`` on the
pydantic-ai Agent.  These tools give Grug read access to a user's character
sheet for context during play — character data is managed externally
(Pathbuilder, D&D Beyond, etc.) and ingested on upload or sync.
"""
